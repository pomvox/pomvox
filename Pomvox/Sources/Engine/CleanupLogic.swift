...
/// Size the cleanup deadline to the transcript (`CleanupDeadline`), then race
/// the pass against it plus a grace period.
///
/// The per-chunk deadline inside `clean()` is authoritative, but a Metal kernel
/// that hangs without ever yielding a chunk would never reach it — the paste
/// must not be held hostage. First result wins; a late one is discarded (the
/// zombie generation can only delay the *next* cleanup, never STT, which runs
/// on the ANE).
///
/// A free function taking the protocol, like `runCleanup`, rather than a method
/// on `NativeEngine`: the deadline arithmetic here decides whether a dictation
/// gets cleaned or pastes raw, and as a private method it could not be tested
/// at all.
func cleanupWithWatchdog(
    _ engine: any CleanupCleaning, raw: String, style: String, timeoutS: Double
) async -> (String, CleanupStatus) {
    // `timeout_s` budgets a TYPICAL utterance; the work is linear in the
    // transcript, so a long one gets proportionally longer. Without this a
    // ~1500-char dictation could not fit any budget the settings slider allows
    // and pasted raw after burning all of it.
    let chars = raw.count
    ...
    if effective > timeoutS {
        NSLog("cleanup: %d chars — deadline widened %.1fs → %.1fs", chars, timeoutS, effective)
    }
    return await withTaskGroup(of: Optional<(String, CleanupStatus)>.self) { group in
        group.addTask {
            await runCleanup(engine, text: raw, style: style, timeoutS: effective)
        }
        group.addTask {
            // Must clear every credit `clean()` may award itself, or this race
            // would cancel a pass still inside its own budget.
