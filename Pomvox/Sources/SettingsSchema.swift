...
enum SettingsSchema {
    ...
    /// Keys the running Swift engine can't hot-apply: the speech model and
    ...
    /// still lists `cleanup.model` for the Python process, which loads models
    /// once at startup; that divergence is intentional. Don't add the cleanup
    /// keys back here unless the native engine stops hot-applying them.
    static let restartRequiredKeys: Set<String> = [
        "hotkey.ptt", "hotkey.toggle", "hotkey.stop", "hotkey.cancel",
        "stt.model", "audio.device", "log.file",
    ]
